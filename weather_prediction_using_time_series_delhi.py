# -*- coding: utf-8 -*-
"""Weather Prediction Using Time Series - Delhi

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1RPx-YrFhUEir5JHG1llD80KF0X0iyxIE
"""
...
# sys.path.append('C:\\Users\\PycharmProjects\\TimeSeriesLabs')
from toolbox import *
...
# Resample by day and calculate mean (excluding 'Datetime')
df = data.resample('D').mean(numeric_only=True)

# Group by date and get min/max for 'temp'
df2 = data.groupby(data.index.date)['temp'].agg(['min', 'max'])

# Add min/max temp columns (avoiding potential key errors)
df['minTemp'] = df2.get('min')  # Use .get to avoid KeyError if 'temp' is missing
df['maxTemp'] = df2.get('max')  # Use .get to avoid KeyError if 'temp' is missing

# Forward fill for missing values (considering 'Datetime' as index)
df = df.ffill()
